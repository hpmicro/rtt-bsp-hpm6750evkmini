...
static uint16_t hid_report_count = 0;
...
static void usbd_event_handler(uint8_t busid, uint8_t event)
{
    ...
    case USBD_EVENT_CONNECTED:
        speed = usbd_get_port_speed(busid);
        if (speed == USB_SPEED_HIGH) {
            hid_report_count = 1024;
            intf0.hid_report_descriptor = hid_custom_report_desc_hs;
            intf0.hid_report_descriptor_len = HID_CUSTOM_REPORT_DESC_SIZE;
        } else {
            hid_report_count = 64;
    ...
    case USBD_EVENT_CONFIGURED:
        ...
        usbd_ep_start_read(busid, HIDRAW_OUT_EP, read_buffer, hid_report_count);
        break;
...
static void usbd_hid_custom_out_callback(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    usbd_ep_start_read(busid, ep, read_buffer, hid_report_count);
