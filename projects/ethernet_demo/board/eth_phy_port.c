...
#if PHY_AUTO_NEGO
    if (reg_status & (PHY_AUTONEGO_COMPLETE_MASK | PHY_LINKED_STATUS_MASK))
    {
        *status = reg_status ? RT_TRUE : RT_FALSE;
    }
#else
    if (reg_status & PHY_LINKED_STATUS_MASK))
    {
        *status = reg_status ? RT_TRUE : RT_FALSE;
    }
#endif
...
static void phy_status_polling(void *parameter)
{
    ...
    if (status)
    {
        ...
        if (ret != 0)
        {
            ...
            if (phy_info.phy_link == PHY_LINK_UP)
            {
                LOG_D("Link up\n");
                LOG_I("%s", phy_info.phy_speed & PHY_SPEED_100M ? "100Mbps" : "10Mbps");
                LOG_I("%s", phy_info.phy_duplex & PHY_FULL_DUPLEX ? "Full Duplex" : "Half Duplex");
                eth_device_linkchange(&eth_dev, RT_TRUE);
            }
            else
            {
                eth_device_linkchange(&eth_dev, RT_FALSE);
            }
        }
    }
}
