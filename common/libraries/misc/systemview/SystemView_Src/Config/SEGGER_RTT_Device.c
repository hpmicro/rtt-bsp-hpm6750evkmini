...
int hw_segger_init(void)
{
    ...
    segger_serial.config.tx_bufsz = RT_SERIAL_RX_MINBUFSZ;
    segger_serial.config.rx_bufsz = RT_SERIAL_RX_MINBUFSZ;
#else
    segger_serial.config.tx_bufsz .bufsz = RT_SERIAL_RB_BUFSZ;
#endif


    /* register segger rtt as serial device */
    rt_hw_serial_register(&segger_serial,
                          RT_SERIAL_DEVICE_NAME,
