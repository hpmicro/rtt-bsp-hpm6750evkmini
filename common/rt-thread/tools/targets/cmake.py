 ...
 * 2025-12-05     HPMicro      fix the issue for getting CFLAGS, CXXFLAGS, ASFLAGS, LINKFLAGS from env
"""
...
def GenerateCFiles(env, project, project_name):
    ...
    CFLAGS = ''.join(env['CFLAGS']).replace('\\', "/").replace('\"', "\\\"")
    if 'CXXFLAGS' in dir(rtconfig):
        cflag_str=''.join(env['CXXFLAGS'])
        CXXFLAGS = cflag_str.replace('\\', "/").replace('\"', "\\\"")
    else:
        CXXFLAGS = CFLAGS
    AFLAGS = ''.join(env['ASFLAGS']).replace('\\', "/").replace('\"', "\\\"")
    LFLAGS = ''.join(env['LINKFLAGS']).replace('\\', "/").replace('\"', "\\\"")
    ...
    # add COMMAND before each command
    ...
    with open("CMakeLists.txt", "w") as cm_file:
        ...
        # auto include `custom.cmake` for user custom settings
        custom_cmake = \
            '''
