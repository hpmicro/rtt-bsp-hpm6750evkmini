 ...
 * 2026-01-15     HPMicro       add support for UHS-I modes
 */
                             ...
                             rt_uint8_t            raw)
{
    ...
    if (reg_addr & ~SDIO_ARG_CMD52_REG_MASK)
        return -RT_ERROR;
...
static rt_int32_t sdio_read_cccr(struct rt_mmcsd_card *card)
{
    ...
    if (cccr_version >= SDIO_CCCR_REV_2_00)
    {
...
static rt_int32_t sdio_set_highspeed(struct rt_mmcsd_card *card)
{
    rt_int32_t ret;
    rt_uint8_t speed;
    rt_uint8_t uhsi_support;
    rt_uint32_t clock_freq;
    rt_uint32_t timing;

    rt_bool_t support_sdr104;
    rt_bool_t support_sdr50;
    rt_bool_t support_ddr50;
    rt_bool_t support_highspeed;
    rt_uint16_t flags;
    ...
    /* UHS-I support is only available in CCCR 3.00 */
    if (card->cccr.sdio_version >= SDIO_SDIO_REV_3_00)
    {
        uhsi_support = sdio_io_readb(card->sdio_function[0], SDIO_REG_CCCR_UHSI_SUPPORT, &ret);
        if (ret)
            return ret;

        support_sdr104 = (uhsi_support & SDIO_SPEED_SSDR104) && (card->host->flags & MMCSD_SUP_SDR104);
        support_sdr50 = (uhsi_support & SDIO_SPEED_SSDR50) && (card->host->flags & MMCSD_SUP_SDR50);
        support_ddr50 = (uhsi_support & SDIO_SPEED_SDDR50) && (card->host->flags & MMCSD_SUP_DDR50);
    }
    else
    {
        support_sdr104 = 0;
    ...
    support_highspeed = (speed & SDIO_SPEED_SHS) && (card->host->flags & MMCSD_SUP_HIGHSPEED);

    speed &= ~(SDIO_SPEED_ESDR104 | SDIO_SPEED_ESDR50 | SDIO_SPEED_EDDR50 | SDIO_SPEED_EHS);

    if (support_sdr104)
    {
        speed |= SDIO_SPEED_ESDR104;
        timing = MMCSD_TIMING_UHS_SDR104;
        clock_freq = 208000000;
        flags = CARD_FLAG_SDR104;
        LOG_D("switch to SDR104 mode");
    }
    else if (support_sdr50)
    {
        speed |= SDIO_SPEED_ESDR50;
        timing = MMCSD_TIMING_UHS_SDR50;
        clock_freq = 100000000;
        flags = CARD_FLAG_SDR50;
        LOG_D("switch to SDR50 mode");
    }
    else if (support_ddr50)
    {
        speed |= SDIO_SPEED_EDDR50;
        timing = MMCSD_TIMING_UHS_DDR50;
        clock_freq = 50000000;
        flags = CARD_FLAG_DDR50;
        LOG_D("switch to DDR50 mode");
    }
    else if (support_highspeed)
    {
        speed |= SDIO_SPEED_EHS;
        timing = MMCSD_TIMING_SD_HS;
        clock_freq = 50000000;
        flags = CARD_FLAG_HIGHSPEED;
        LOG_D("switch to HighSpeed mode");
    }
    else
    {
        timing = MMCSD_TIMING_LEGACY;
        clock_freq = 25000000;
        flags = 0;
    }
    ...
    mmcsd_set_clock(card->host, clock_freq);

    mmcsd_set_timing(card->host, timing);
