...
void bldc_foc_al_speed(BLDC_CONTRL_SPD_PARA  *par)
{
    ...
    if(deta > HPM_MATH_FL_MDF(180)){/*-speed*/
        deta = -par->speedlasttheta - (HPM_MATH_FL_MDF(360) - par->speedtheta);
    }
    else if(deta < HPM_MATH_FL_MDF(-180)){/*+speed*/
        deta = HPM_MATH_FL_MDF(360) + par->speedtheta - par->speedlasttheta;
    }
    ...
    if(par->I_speedacq == par->num){
        par->num = 0;
        par->O_speedout = HPM_MATH_DIV(par->speedthetaLastN,
            HPM_MATH_MUL(HPM_MATH_MUL( par->I_speedLooptime_s,HPM_MATH_FL_MDF(par->I_motorpar->I_Poles_n)),HPM_MATH_FL_MDF(360)));
        par->O_speedout_filter = par->O_speedout_filter + HPM_MATH_MUL(par->I_speedfilter,
...
** ===================================================================
*/
static HPM_MATH_TYPE bldc_foc_sin(HPM_MATH_TYPE angle , HPM_MATH_TYPE angle_precision)
{
     HPM_MATH_TYPE result=0;
     int16_t  transfer=0;
     if((angle >= 0) && (angle <= HPM_MATH_FL_MDF(90)))
     {
        transfer = HPM_MATH_MDF_FL(HPM_MATH_DIV(angle , angle_precision));
        result = HPM_MATH_FL_MDF(bldc_foc_sintable[transfer]);
     }
     else if((angle > HPM_MATH_FL_MDF(90)) && (angle <= HPM_MATH_FL_MDF(180)))
     {
        transfer = HPM_MATH_MDF_FL(HPM_MATH_DIV((HPM_MATH_FL_MDF(180) - angle),angle_precision));
        result = HPM_MATH_FL_MDF(bldc_foc_sintable[transfer]);
     }
     else if((angle > HPM_MATH_FL_MDF(180)) && (angle <= HPM_MATH_FL_MDF(270)))
     {
        transfer = HPM_MATH_MDF_FL(HPM_MATH_DIV((angle - HPM_MATH_FL_MDF(180)),angle_precision));
        result = HPM_MATH_FL_MDF(-bldc_foc_sintable[transfer]);
     }
     else if((angle > HPM_MATH_FL_MDF(270)) && (angle <= HPM_MATH_FL_MDF(360)))
     {
        transfer = HPM_MATH_MDF_FL(HPM_MATH_DIV((HPM_MATH_FL_MDF(360)-angle),angle_precision));
        result = HPM_MATH_FL_MDF(-bldc_foc_sintable[transfer]);
     }
     return(result);
}
...
** ===================================================================
*/
static HPM_MATH_TYPE bldc_foc_cos(HPM_MATH_TYPE angle , HPM_MATH_TYPE angle_precision)
{
     HPM_MATH_TYPE result=0;
     int16_t  transfer=0;
     if((angle >= 0) && (angle <= HPM_MATH_FL_MDF(90)))
     {
        transfer = HPM_MATH_MDF_FL(HPM_MATH_DIV((HPM_MATH_FL_MDF(90) - angle),angle_precision));
        result = HPM_MATH_FL_MDF(bldc_foc_sintable[transfer]);
     }
     else if((angle > HPM_MATH_FL_MDF(90)) && (angle <= HPM_MATH_FL_MDF(180)))
     {
        transfer = HPM_MATH_MDF_FL(HPM_MATH_DIV((angle - HPM_MATH_FL_MDF(90)),angle_precision));
        result = HPM_MATH_FL_MDF(-bldc_foc_sintable[transfer]);
     }
     else if((angle > HPM_MATH_FL_MDF(180)) && (angle <= HPM_MATH_FL_MDF(270)))
     {
        transfer = HPM_MATH_MDF_FL(HPM_MATH_DIV((HPM_MATH_FL_MDF(270) - angle),angle_precision));
        result = HPM_MATH_FL_MDF(-bldc_foc_sintable[transfer]);
     }
     else if((angle > HPM_MATH_FL_MDF(270)) && (angle <= HPM_MATH_FL_MDF(360)))
     {
        transfer =HPM_MATH_MDF_FL(HPM_MATH_DIV((angle - HPM_MATH_FL_MDF(270)),angle_precision));
        result = HPM_MATH_FL_MDF(bldc_foc_sintable[transfer]);
     }
...
{
    HPM_MATH_TYPE sin_angle,cos_angle;
    sin_angle = bldc_foc_sin(angle,HPM_MATH_FL_MDF(PRECISION));
    cos_angle = bldc_foc_cos(angle,HPM_MATH_FL_MDF(PRECISION));
    *ualpha = HPM_MATH_MUL(cos_angle , ud) + HPM_MATH_MUL(-sin_angle , uq); // PARK逆变换
    *ubeta = HPM_MATH_MUL(sin_angle , ud) + HPM_MATH_MUL(cos_angle , uq);
}
...
{
    HPM_MATH_TYPE ualpha_60, ubeta_30;
    uint32_t pwm_reload;
    ...
    /*求出参考电压*/
    uref1 = HPM_MATH_MDF_FL(par->target_beta);
    ualpha_60 = HPM_MATH_MUL(HPM_MATH_FL_MDF(0.866025 ) , par->target_alpha);
    ubeta_30 = HPM_MATH_MUL(par->target_beta , HPM_MATH_FL_MDF(0.5));
    uref2 = HPM_MATH_MDF_FL(ualpha_60 - ubeta_30);   
    uref3 = HPM_MATH_MDF_FL(-ualpha_60 - ubeta_30);
...
{
    HPM_MATH_TYPE curbeta;
    *currentAlpha = currentU;
    curbeta = HPM_MATH_MUL(HPM_MATH_FL_MDF(0.5773502) , currentU)  + HPM_MATH_MUL(HPM_MATH_FL_MDF(1.1547004), currentV);
    *currentBeta = curbeta;
                   ...
                   HPM_MATH_TYPE * currentd, HPM_MATH_TYPE * currentq)
{
    ...
    *currentd = HPM_MATH_MUL(cos_angle , currentAlpha) + HPM_MATH_MUL(sin_angle , currentBeta);
    *currentq = HPM_MATH_MUL(-sin_angle , currentAlpha) + HPM_MATH_MUL(cos_angle , currentBeta);
}
/*
** ===================================================================
...
{
    par->cal_u = HPM_MATH_FL_MDF(par->adc_u_middle - par->adc_u);
    par->cal_v = HPM_MATH_FL_MDF(par->adc_v_middle - par->adc_v);
    par->cal_w=  HPM_MATH_FL_MDF(-(par->cal_u + par->cal_v));
...
{
   HPM_MATH_TYPE result = 0;//PI运算结果

   /*目标电流和采样电流进行PI调节，计算给定电压*/
   HPM_MATH_TYPE curerr = 0;       
   HPM_MATH_TYPE portion_asp = 0;  
   HPM_MATH_TYPE portion_asi = 0;  

   curerr = par->target - par->cur;
   portion_asp = HPM_MATH_MUL(curerr , (par->I_kp));
   portion_asi = HPM_MATH_MUL(curerr , (par->I_ki))+ par->mem;
