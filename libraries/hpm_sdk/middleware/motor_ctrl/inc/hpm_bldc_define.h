...
typedef struct bldc_motor_par{
    ...
    /*输出参数*/
    HPM_MATH_TYPE  O_smc_f;			                /* 滑膜控制系数1 */
    HPM_MATH_TYPE  O_smc_g;			                /* 滑膜控制系数2*/
    /*电机参数相关的函数*/
...
typedef struct  bldc_contrl_spd_par{
    ...
    HPM_MATH_TYPE       I_speedLooptime_s;        /*循环n次的时间 s*/
    HPM_MATH_TYPE       speedtheta;             /*计算速度使用的角度*/
    HPM_MATH_TYPE       speedlasttheta;          /*存储上次的角度值*/
    HPM_MATH_TYPE       speedthetaLastN;        /*累加theta，初始化为0即可，用户不需要关心*/
    HPM_MATH_TYPE       I_speedfilter;            /*速度滤波系数，低通滤波器*/
    HPM_MATH_TYPE       O_speedout_filter;        /*滤波后速度输出*/
    HPM_MATH_TYPE       O_speedout;               /*直接速度输出*/
...
typedef struct bldc_contrl_pid_par{
    HPM_MATH_TYPE       I_kp;                   /*Kp*/
    HPM_MATH_TYPE       I_ki;                   /*Ki*/
    HPM_MATH_TYPE       I_kd;                   /*Kd*/
    HPM_MATH_TYPE       I_max;                  /*最大值  默认min = -max*/
    HPM_MATH_TYPE       target;                 /*目标*/
    HPM_MATH_TYPE       mem;                    /*内部运算使用*/
    HPM_MATH_TYPE       cur;                    /*采集*/
    HPM_MATH_TYPE       outval;                 /*输出*/
...
typedef struct bldc_contrl_current_par{
    ...
    HPM_MATH_TYPE       cal_u;                  /*计算后的U相电流*/
    HPM_MATH_TYPE       cal_v;                  /*计算后的v相电流*/
    HPM_MATH_TYPE       cal_w;                  /*计算后的W相电流*/
...
typedef struct bldc_control_pwm_par{
    HPM_MATH_TYPE       target_alpha;
    HPM_MATH_TYPE       target_beta;
...
typedef struct bldc_control_smc_par{
    HPM_MATH_TYPE       I_Ezero;    /*hua*/
    HPM_MATH_TYPE       I_ksmc;  
    HPM_MATH_TYPE       I_kfil;     
    HPM_MATH_TYPE       *ualpha;
    HPM_MATH_TYPE       *ubeta;     	       		
    HPM_MATH_TYPE       *ialpha;
    HPM_MATH_TYPE       *ibeta;  
    /*内部运算*/
    HPM_MATH_TYPE       ialpha_mem;
    HPM_MATH_TYPE       ibeta_mem;            
    HPM_MATH_TYPE       alpha_cal;   	
    HPM_MATH_TYPE       zalpha_cal; 
    HPM_MATH_TYPE       beta_cal;  	
    HPM_MATH_TYPE       zbeta_cal;  
    /*输出*/    
    HPM_MATH_TYPE       *theta;
    BLDC_MOTOR_PARA     *I_motorpar;                /*电机参数*/ 
    void (*func_smc)();
...
typedef struct bldc_contrl_foc_par{
    ...
    HPM_MATH_TYPE               electric_angle;             /*当前电角度*/
    ...
    HPM_MATH_TYPE               u_alpha;
    HPM_MATH_TYPE               u_beta;
    HPM_MATH_TYPE               i_alpha;
    HPM_MATH_TYPE               i_beta;
    void (*func_dqsvpwm)();
