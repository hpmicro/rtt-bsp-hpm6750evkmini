    ...
    do {
        ...
        if (card->sd_flags.support_set_block_count_cmd != 0) {
            while (block_count > 0) {
                ...
                content->data = data;
                content->command = cmd;
                l1c_dc_flush((uint32_t) data->rx_data, card->block_size * block_count);
                status = sdmmchost_transfer(card->host, content);
                l1c_dc_flush((uint32_t) data->rx_data, card->block_size * block_count);
                if (status != status_success) {
                    break;
            ...
            while (block_count > 0) {
                ...
                content->data = data;
                content->command = cmd;
                l1c_dc_flush((uint32_t) data->rx_data, card->block_size);
                status = sdmmchost_transfer(card->host, content);
                l1c_dc_invalidate((uint32_t) data->rx_data, card->block_size);
                if (status != status_success) {
                    break;
                }
                block_count--;
    ...
    do {
        ...
        if (card->sd_flags.support_set_block_count_cmd != 0) {
            while (block_count > 0) {
                ...
                content->data = data;
                content->command = cmd;
                l1c_dc_flush((uint32_t) data->tx_data, card->block_size * write_block_count);
                status = sdmmchost_transfer(card->host, content);
                if (status != status_success) {
                    break;
                }
            ...
            while (block_count > 0) {
                ...
                content->data = data;
                content->command = cmd;
                l1c_dc_flush((uint32_t) data->tx_data, card->block_size);
                status = sdmmchost_transfer(card->host, content);
                if (status != status_success) {
                    break;
                }
